...
def get_hig_moving_items(preprocessed_data):
    ...
    item_volume_share = preprocessed_data.groupby('StoreId')['item_weekly_volumn'].sum().reset_index()
    item_volume_share.rename(columns={'item_weekly_volumn': 'store_volume_share'}, inplace=True)
    item_volume= preprocessed_data.groupby(['StoreId','MenuItemName'])['item_weekly_volumn'].sum().reset_index()

    high=item_volume.merge(item_volume_share, on='StoreId', how='left', validate='m:1')
    ...
    high['store_75th'] = high.groupby('StoreId')['volume_share'].transform(lambda x: x.quantile(0.75))
    ...
    high_volume_items=preprocessed_data[preprocessed_data['MenuItemName'].isin(high_items['MenuItemName'])]
    print('number of items in high volume share items',high_volume_items.MenuItemName.nunique())

    # looking for top 1o items in each store
    top10_items_per_store = high.sort_values(['StoreId', 'volume_share'], ascending=[True, False]) \
                                .groupby('StoreId').head(10)
    ...
    # # Get top 30 items
    top_items_all = top10_items_per_store.sort_values(by='volume_share', ascending=False)
...
def declining_items(high_volume_items):
    
    # Prepare a results list
    results = []

    # Group by StoreId and OrderType
    for (store_id, order_type), group in high_volume_items.groupby(['StoreId', 'OrderType']):
        trend_data = []
    ...
    print('number of unique declining items', top_declining_items['MenuItemName'].nunique())
    
    df_tren = high_volume_items.merge(top_declining_items, on=['StoreId', 'OrderType', 'MenuItemName'])
    df_tren.head(5)
...
result=declining_items(high_volume_items)
...
def show_graph(filtered_df,weeks,months):
    ...
    fig.update_yaxes(matches=None)
    fig.write_image(f"./output/decline_{weeks}_weeks_{months}_months.png", width=1200, height=800)
    fig.show()
...
def x_weeks_consecutive_decline_y_month(df, weeks=4, months=3,show_graphs=True):
    ...
    df_recent = df.sort_values(['StoreId', 'OrderType', 'MenuItemName', 'WeekStartDate'])
...
months=3
show_graphs=True
week_4_consective_decline=x_weeks_consecutive_decline_y_month(high_volume_items, weeks, months,show_graphs)



high_volume_items.groupby(['StoreId', 'OrderType', 'MenuItemName'])['item_weekly_volumn'].sum().reset_index().sort_values(by='item_weekly_volumn', ascending=False)
week_4_consective_decline['MenuItemName'].unique()
week_4_consective_decline[(week_4_consective_decline['MenuItemName']=='PARADISE ACAI BOWL') & (week_4_consective_decline['StoreId']==963290) & (week_4_consective_decline['OrderType'].isin(['Take Out','Online-Dispatch']))].sort_values(by=['OrderType','WeekStartDate'])
# 
...
week_4_consective_decline[(week_4_consective_decline['MenuItemName']=='POKE BOWL') & (week_4_consective_decline['StoreId']==963290) & (week_4_consective_decline['OrderType'].isin(['Online-Pickup']))].sort_values(by=['OrderType','WeekStartDate'])['item_weekly_volumn']
...
week_4_consective_decline=x_weeks_consecutive_decline_y_month(high_volume_items, weeks, months,show_graphs)
week_4_consective_decline.MenuItemName.unique()
week_4_consective_decline[(week_4_consective_decline['MenuItemName']=='ALMOND BUTTER ACAI BOWL') & (week_4_consective_decline['StoreId']==963290) & (week_4_consective_decline['OrderType'].isin(['Take Out']))].sort_values(by=['OrderType','WeekStartDate'])
